import { DayEvents,type DayEvent } from '../day-events'
...
export function CalendarPage() {
  const [selected,setSelected]=useState<DayEvent[]>([])
  const [customers,setCustomers]=useState<Record<string,string>>({})
  const [cursor, setCursor] = useState(() => monthStart(new Date()))
  const [activities, setActivities] = useState<ProjectActivity[]>([])
  const [projects, setProjects] = useState<Project[]>([])
  const [reminders, setReminders] = useState<Reminder[]>([])
  useEffect(() => { void Promise.all([activityRepository.getAll(), projectRepository.getAll(), reminderRepository.getOpen(),customerRepository.getAll()]).then(([allActivities, projectList, reminderList,customerList]) => { setCustomers(Object.fromEntries(customerList.map(c=>[c.id,c.name])));setActivities(allActivities); setProjects(projectList); setReminders(reminderList) }) }, [])
  ...
  const projectLabel=(id?:string)=>{const p=projects.find(p=>p.id===id);return p?`${p.title} — ${customers[p.customerId]||"مشتری نامشخص"}`:"بدون پروژه"}
  return <div className="space-y-5">{selected.length>0&&<DayEvents events={selected} onClose={()=>setSelected([])} onSelect={event=>setSelected([event])}/>}<div><h1 className="text-2xl font-bold">تقویم کارها</h1><p className="mt-1 text-sm text-slate-500">فعالیت‌ها، یادآوری‌ها و موعد پروژه‌ها در یک نما.</p></div><section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><header className="flex items-center justify-between border-b border-slate-100 p-4"><button aria-label="ماه بعد" onClick={() => setCursor(shiftMonth(cursor, 1))} className="rounded-lg p-2 hover:bg-slate-100"><ChevronRight size={20} /></button><div className="text-center"><h2 className="font-bold">{faMonth.format(cursor)}</h2><button onClick={() => setCursor(monthStart(new Date()))} className="mt-1 text-xs text-indigo-600">بازگشت به امروز</button></div><button aria-label="ماه قبل" onClick={() => setCursor(shiftMonth(cursor, -1))} className="rounded-lg p-2 hover:bg-slate-100"><ChevronLeft size={20} /></button></header><div className="grid grid-cols-7 border-b border-slate-100">{weekdays.map((day) => <div key={day} className="py-2 text-center text-xs font-semibold text-slate-500">{day}</div>)}</div><div className="grid grid-cols-7">{days.map((day) => { const key = dateKey(day); const inMonth = isoToJalali(key).month === isoToJalali(dateKey(cursor)).month; const dayActivities = activityMap[key] || []; const dayReminders = reminderMap[key] || []; const dayProjects = plannedMap[key] || []; const events:DayEvent[]=[...dayActivities.map(a=>({id:a.id,kind:"فعالیت" as const,title:a.title,detail:projectLabel(a.projectId)+"\n"+(a.note||"")+(a.quantity!==undefined?"\nمقدار: "+a.quantity:""),projectId:a.projectId,date:key})),...dayReminders.map(r=>({id:r.id,kind:"یادآوری" as const,title:r.title,detail:projectLabel(r.projectId)+"\n"+(r.note||""),projectId:r.projectId,date:key})),...dayProjects.map(p=>({id:p.id,kind:"موعد پروژه" as const,title:p.title,detail:projectLabel(p.id),projectId:p.id,date:key}))]; return <button type="button" aria-label={key+"، "+events.length+" رویداد"} disabled={!events.length} onClick={()=>setSelected(events)} key={key} className={`min-h-24 border-b border-l border-slate-100 p-1.5 sm:min-h-32 ${inMonth ? 'bg-white' : 'bg-slate-50/70'}`}><span className={`grid h-6 w-6 place-items-center rounded-full text-xs ${key === today ? 'bg-indigo-600 font-bold text-white' : inMonth ? 'text-slate-700' : 'text-slate-400'}`}>{new Intl.DateTimeFormat('fa-IR', { day: 'numeric' }).format(day)}</span><div className="mt-1 space-y-1">{dayReminders.slice(0, 2).map((item) => <p key={item.id} title={item.title} className="truncate rounded bg-amber-50 px-1 py-0.5 text-[10px] text-amber-800">{item.title}</p>)}{dayProjects.slice(0, 1).map((item) => <p key={item.id} title={item.title} className="truncate rounded bg-rose-50 px-1 py-0.5 text-[10px] text-rose-700">موعد: {item.title}</p>)}{dayActivities.length > 0 && <p className="truncate rounded bg-emerald-50 px-1 py-0.5 text-[10px] text-emerald-700">{dayActivities.length} فعالیت</p>}</div></button> })}</div></section><p className="text-xs text-slate-500"><span className="text-amber-700">زرد:</span> یادآوری · <span className="text-rose-700">قرمز:</span> موعد پروژه · <span className="text-emerald-700">سبز:</span> فعالیت ثبت‌شده</p></div>
}
