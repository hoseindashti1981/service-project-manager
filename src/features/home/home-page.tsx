...
export function HomePage() {
  ...
  const colors = ['bg-slate-600', 'bg-sky-600', 'bg-indigo-600', 'bg-amber-600', 'bg-emerald-600']
  return <div className="space-y-6">
    ...
    <section aria-label="وضعیت پروژه‌ها" className="grid grid-cols-2 gap-3 lg:grid-cols-5">{dashboardStatuses.map((status, index) => <Link key={status} to="/projects" search={{ status }} className={`${colors[index]} rounded-2xl p-5 text-white shadow-sm hover:opacity-90 focus-visible:outline-2`}><p className="text-sm">پروژه‌های {projectStatusLabels[status]}</p><p className="mt-3 text-2xl font-bold">{data.projects.filter((item) => item.status === status).length.toLocaleString('fa-IR')}</p><p className="mt-2 text-xs text-white/80">مشاهده پروژه‌ها ←</p></Link>)}</section>
