...
export function TodayActivitiesPage() {
  ...
  const reset = () => { setTitle(''); setQuantity(''); setNote(''); setItemId(''); setPlannedQuantity(''); setEditing(null) }
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (itemId.startsWith('catalog:')) await saveCatalogActivity(input, itemId.slice('catalog:'.length), Number(normalizeDigits(plannedQuantity)), editing || undefined)
      else if (editing) await activityRepository.update(editing, input)
