...
export function TodayActivitiesPage() {
  ...
  const reset = () => { setStartExecution(false); setHistorical(false); setAuditReason(''); setTitle(''); setQuantity(''); setNote(''); setItemId(''); setPlannedQuantity(''); setEditing(null) }
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const input = { projectId: selectedProject, projectItemId: selectedService ? undefined : itemId || undefined, date, title: title.trim(), quantity: quantity.trim() ? Number(normalizeDigits(quantity)) : undefined, unit: selectedUnit, note }
      if (itemId.startsWith('catalog:')) await saveCatalogActivity(input, itemId.slice('catalog:'.length), Number(normalizeDigits(plannedQuantity)), editing || undefined, options)
  ...
  return <div className="space-y-6">
    ...
    <form onSubmit={(event) => void submit(event)} className="rounded-2xl border bg-white p-4">
      <fieldset disabled={saving || loading} className="space-y-3"><legend className="mb-3 font-bold">{editing ? 'ویرایش فعالیت' : 'ثبت سریع فعالیت'}</legend>
        ...
        <label className="block">مقدار انجام‌شده {selectedUnit ? `(${serviceUnits.find((unit) => unit.value === selectedUnit)?.label})` : '(اختیاری)'}<input required={!!itemId} inputMode="decimal" value={quantity} onChange={(event) => setQuantity(normalizeDigits(event.target.value))} className={field} /></label>
