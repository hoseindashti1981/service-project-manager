...
export function TodayActivitiesPage() {
  ...
  const [highlightedActivityId,setHighlightedActivityId]=useState(()=>window.location.hash.startsWith('#activity-')?decodeURIComponent(window.location.hash.slice(10)):'')
  useEffect(()=>{void db.appSettings.get("business").then(settings=>setAllDates(window.location.hash.startsWith("#activity-")||settings?.defaultActivityRange!=="today"))},[])
  useEffect(()=>{if(loading||!highlightedActivityId)return;const target=document.getElementById('activity-'+highlightedActivityId);if(!target)return;target.scrollIntoView({block:'center'});const timer=window.setTimeout(()=>{setHighlightedActivityId('');history.replaceState(null,'',location.pathname+location.search)},1800);return()=>window.clearTimeout(timer)},[loading,activities,highlightedActivityId])
  useEffect(() => {
  ...
  const reset = () => { setFormOpen(false); setAttachPricedService(false); setAmount(''); setStartExecution(false); setHistorical(false); setAuditReason(''); setTitle(''); setQuantity(''); setNote(''); setItemId(''); setPlannedQuantity(''); setEditing(null) }
  ...
  return <div className="flex flex-col gap-6">
    <div className="order-first flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-bold">فعالیت‌های روزانه</h2><Link to="/projects" className="text-sm text-indigo-600">پروژه‌ها</Link></div>
    <button className="order-first min-h-11 self-start rounded-xl bg-indigo-700 px-4 text-white" onClick={()=>{reset();setFormOpen(true)}}>فعالیت جدید</button>
    {formOpen && <form onSubmit={(event) => void submit(event)} className="order-first rounded-2xl border bg-white p-4">
      <fieldset disabled={saving || loading} className="space-y-3"><legend className="mb-3 font-bold">{editing ? 'ویرایش فعالیت' : 'ثبت سریع فعالیت'}</legend>
        ...
        {currentProject && <div className="space-y-2 rounded-xl bg-slate-50 p-3 text-sm"><p>وضعیت پروژه: {projectStatusLabels[currentProject.status]}</p>
          ...
          {(editing || historical) && <label className="block">دلیل اصلاح یا ثبت دیرهنگام<textarea rows={4} aria-label="دلیل اصلاح یا ثبت دیرهنگام" required value={auditReason} onChange={(event) => setAuditReason(event.target.value)} className={field} /></label>}
        </div>}
        ...
        <label className="block">یادداشت<textarea aria-label="یادداشت" rows={4} value={note} onChange={(event) => setNote(event.target.value)} className={field} /></label>
    ...
    <section className="order-last space-y-3"><h3 className="font-bold">پیشرفت خدمات — {selectedProject ? projectName(selectedProject) : 'پروژه‌ای ثبت نشده'}</h3>{projectItems.length === 0 && <p className="text-sm text-slate-500">از فهرست خدمات ذخیره‌شده بالا انتخاب کنید یا در صفحه پروژه خدمت اضافه کنید.</p>}{projectItems.map((item) => { const progress = serviceProgress(item, activities); return <article key={item.id} className="space-y-2 rounded-xl border bg-white p-3"><b>{item.title}</b><p className="text-sm">انجام‌شده: {progress.completed.toLocaleString('fa-IR')} از {item.quantity.toLocaleString('fa-IR')} · باقی‌مانده: {progress.remaining.toLocaleString('fa-IR')} {serviceUnits.find((unit) => unit.value === item.unit)?.label}</p><progress aria-label={`پیشرفت ${item.title}`} value={progress.percent} max={100} className="w-full" />{progress.extra > 0 && <p className="text-xs text-amber-700">مازاد بر مقدار برنامه: {progress.extra.toLocaleString('fa-IR')}</p>}</article> })}</section>
    <section className="space-y-3"><div className="flex flex-wrap justify-between gap-3"><h3 className="font-bold">{allDates ? 'همه فعالیت‌ها' : `فعالیت‌های ${formatDateFa(date)}`}</h3><label className="text-sm"><input type="checkbox" checked={allDates} onChange={(event) => setAllDates(event.target.checked)} /> نمایش همه تاریخ‌ها</label></div>{loading ? <p>در حال بارگذاری…</p> : visible.length === 0 ? <p className="text-sm text-slate-500">فعالیتی ثبت نشده است.</p> : visible.map((activity) => <article id={"activity-"+activity.id} key={activity.id} className={`${highlightedActivityId===activity.id?'activity-target-flash ':''}activity-target space-y-2 rounded-xl border bg-white p-4`}><b>{activity.title}</b><p className="text-sm text-slate-500">{projectName(activity.projectId)} · {formatDateFa(activity.date)}</p>{activity.quantity !== undefined && <p className="text-sm">مقدار: {activity.quantity.toLocaleString('fa-IR')} {serviceUnits.find((unit) => unit.value === activity.unit)?.label}</p>}{activity.amount !== undefined && <p className="text-sm">مبلغ یادداشت‌شده: {formatMoney(activity.amount)}</p>}{activity.note && <p className="text-sm">{activity.note}</p>}<div className="flex gap-4 text-sm"><button disabled={saving} onClick={() => { setAuditReason(''); setHistorical(false); setStartExecution(false); setAttachPricedService(false); setAmount(activity.amount === undefined ? '' : String(activity.amount)); setFormOpen(true); setEditing(activity.id); setProjectId(activity.projectId); setItemId(activity.projectItemId || ''); setDate(activity.date); setTitle(activity.title); setQuantity(activity.quantity === undefined ? '' : String(activity.quantity)); setNote(activity.note || ''); setError(''); setMessage(''); window.scrollTo({ top: 0, behavior: 'smooth' }) }}>ویرایش</button><button disabled={saving} onClick={() => void remove(activity)} className="text-rose-600">حذف</button></div></article>)}</section>
  </div>
}
