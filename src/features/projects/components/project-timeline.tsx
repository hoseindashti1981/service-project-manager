...
type Entry = { activityId?: string; id: string; date: string; title: string; detail: string; tone: string; recordedAt?: number }
...
export function ProjectTimeline({ projectId, createdAt, startDate }: { projectId: ID; createdAt: number; startDate?: string }) {
  ...
  return <section className="rounded-2xl border bg-white p-5"><h3 className="font-bold">تاریخچه پروژه</h3>{error && <p role="alert">{error}</p>}<div className="mt-4 space-y-4">{entries.map((item) => <div key={item.id} className="relative border-r-2 border-slate-100 pr-5"><span className={`absolute -right-1.5 top-1 h-3 w-3 rounded-full ${item.tone}`} /><p className="text-xs text-slate-500">تاریخ مؤثر: {formatDateFa(item.date)}</p>{item.activityId ? <Link to="/activities/today" hash={"activity-"+item.activityId} className="inline-flex min-h-11 items-center text-sm font-bold text-indigo-700 underline underline-offset-4">{item.title}</Link> : <b className="text-sm">{item.title}</b>}<p className="mt-1 text-xs text-slate-600">{item.detail}</p>{item.recordedAt && <p className="mt-1 text-xs text-slate-400">زمان ثبت: {new Intl.DateTimeFormat('fa-IR', { dateStyle: 'short', timeStyle: 'short' }).format(item.recordedAt)}</p>}</div>)}</div></section>
}
