...
try{
 ...
 assert.equal(manifest.name,'لاین‌یار');assert.equal(manifest.short_name,'لاین‌یار');assert.equal(manifest.display,'standalone');assert.equal(manifest.id,manifest.start_url);assert.equal(manifest.scope,base)
 assert.equal(await page.locator('meta[name="apple-mobile-web-app-capable"]').getAttribute('content'),'yes')
 assert.equal(await page.locator('meta[name="apple-mobile-web-app-title"]').getAttribute('content'),'لاین‌یار')
 const cdp=await context.newCDPSession(page);const parsed=await cdp.send('Page.getAppManifest');assert.deepEqual(parsed.errors,[]);await cdp.detach()
 await page.getByText(/این پنجره داخل مرورگر باز شده است/).waitFor()
 await page.evaluate(()=>{Object.defineProperty(navigator,'standalone',{value:true,configurable:true});window.dispatchEvent(new Event('pageshow'))})
 await page.getByText('این پنجره در حالت اپ مستقل باز شده است.',{exact:true}).waitFor()
 await page.evaluate(()=>{Object.defineProperty(navigator,'standalone',{value:false,configurable:true});window.dispatchEvent(new Event('pageshow'))})
