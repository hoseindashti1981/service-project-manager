...
try{
 ...
 await page.getByRole('button',{name:'اشتراک‌گذاری موقعیت',exact:true}).click();assert.match(await page.evaluate(()=>window.shared.url),/destination=35.689200/)
 ...
 assert.equal(await page.locator('#photos').evaluate(e=>e.open),true);const galleryTop=await page.getByRole('button',{name:'نمایش photo1',exact:true}).boundingBox();assert.ok(galleryTop.y<650);assert.equal(await page.getByLabel('شرح عکس').isVisible(),false)
