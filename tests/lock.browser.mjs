...
try{
 ...
 await page.getByRole('button',{name:'تنظیمات قفل ورود',exact:true}).click()
 ...
 const second=await context.newPage();await second.goto(base+'/settings')
 ...
 await page.getByRole('button',{name:'تنظیمات قفل ورود',exact:true}).waitFor()
 await page.reload()
 await page.getByRole('button',{name:'تنظیمات قفل ورود',exact:true}).waitFor()
